...
const CategoryTable: React.FC<CategoryTableProps> = ({
  ...
  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-4">
                ...
                {expandedCategories[category.category_id] && (
                  <tr>
                    <td colSpan={5} className="px-0 py-0 border-0">
                      <div className="bg-blue-50 py-4 border-l-4 border-blue-400">
                        <div className="px-6 py-2 border-l-4 border-blue-500 ml-4">
                        ...
                        {sections[category.category_id] && sections[category.category_id].filter(s => s.status === 0).length > 0 && (
                          <div className="mt-4 px-10 text-right">
                            <span className="text-xs text-gray-500">
                              {sections[category.category_id].filter(s => s.status === 0).length} secciones no visibles
                            </span>
                          </div>
                        )}
                      </div>
      ...
      {/* Botón para agregar nueva categoría dentro de la tabla */}
